...
export async function main(ns) {
  ...
  while (true){
    ...
    if (usedram < ns.getServerMaxRam(host) - scriptram * ns.args[0]){
      ...
      if (rand <= prob1){
        ...
        for (let i = 0; i < targets.length; ++i){
          ...
          if (targets[i]["hackwaggle"] >= servercheck){
            ...
            obj["port"] = port;
            hackports.push(obj);
            ns.exec("/dev/worker.js", host, ns.args[0], "hack", targets[i]["name"], port);
            port += 1;
            usedram += scriptram;
          }
        }
        ...
        for (let i = 0; i < targets.length; ++i){
          ...
          if (targets[i]["growwaggle"] >= servercheck){
            ...
            obj["port"] = port;
            growports.push(obj);
            ns.exec("/dev/worker.js", host, ns.args[0], "grow", targets[i]["name"], port);
            port += 1;
          }
        }
      } else {
        ...
        for (let i = 0; i < targets.length; ++i){
          ...
          if (targets[i]["weakenwaggle"] >= servercheck){
            ...
            ns.exec("/dev/worker.js", host, ns.args[0], "weaken", targets[i]["name"], port);
            port += 1;
          }
        }
      }
